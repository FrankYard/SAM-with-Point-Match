...
import utils.pips_basic
from utils.pips_basic import print_stats
import utils.pips_samp
import utils.pips_misc
...
def balanced_ce_loss(pred, gt, valid=None):
    ...
    pos_loss = utils.pips_basic.reduce_masked_mean(loss, pos*valid)
    neg_loss = utils.pips_basic.reduce_masked_mean(loss, neg*valid)
...
def sequence_loss(flow_preds, flow_gt, vis, valids, gamma=0.8):
    ...
    for i in range(n_predictions):
        ...
        flow_loss += i_weight * utils.pips_basic.reduce_masked_mean(i_loss, valids)
...
class DeltaBlock(nn.Module):
    ...
    def forward(self, fhid, fcorr, flow):
        B, S, D = flow.shape
        assert(D==3)
        flow_sincos = utils.pips_misc.get_3d_embedding(flow, 64, cat_coords=True)
...
class Pips(nn.Module):
    ...
    def forward(self, xys, rgbs, coords_init=None, feat_init=None, iters=3, trajs_g=None, vis_g=None, valids=None, sw=None, return_feat=False, is_train=False):
        ...
        if feat_init is None:
            ...
            ffeat = utils.pips_samp.bilinear_sample2d(fmaps[:,0], coords[:,0,:,0], coords[:,0,:,1]).permute(0, 2, 1) # B, N, C
        else:
            ffeat = feat_init
        ...
        if sw is not None and sw.save_this:
            ...
            fcps_ = utils.pips_basic.normalize(fcps_)
            for s in range(S):
